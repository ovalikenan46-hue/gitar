...
export function StrumPattern() {
  ...
  useEffect(() => {
    ...
    // Rough tempo ~ 80 BPM -> ~750ms per beat.
    // For 8th notes (down/up), we'll just map to array index.
  ...
  return (
    <div className="w-full bg-white rounded-3xl p-6 shadow-sm border border-border">
      <div className="flex gap-4 mb-8 justify-center">
        <Button size="icon" className="rounded-xl w-12 h-12" onClick={() => setIsPlaying(!isPlaying)}>
          {isPlaying ? <Square className="w-6 h-6" /> : <Play className="w-6 h-6 ml-1" />}
        </Button>
        <Button size="icon" variant="outline" className="rounded-xl w-12 h-12" onClick={() => { setIsPlaying(false); setCurrentBeat(-1); }}>
